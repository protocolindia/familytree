...
function AdminTreesView({ trees, setTrees, onOpenTree }) {
  ...
  useEffect(()=>{Promise.all([api("GET","/api/admin/trees"),api("GET","/api/masterdata/villages"),api("GET","/api/masterdata/surnames")]).then(([t,v,s])=>{setTrees(t);setVil(v);setSur(s);}).catch(()=>{}).finally(()=>setLoad(false));},[]); 
  const createTree=async()=>{if(!treeName)return;setSave(true);try{const t=await api("POST","/api/trees",{name:treeName,nameEn:treeName,visibility:vis});setTrees(v=>[t,...v]);setNew(false);setVillage("");setSurname("");setTreeNo("1");flash("Tree created!");}catch(e){flash("Error: "+e.message);}setSave(false);};
    ...
    <div className="sidebar">
      ...
      <nav style={{ flex:1, overflowY:"auto", padding:"8px 8px" }}>
        ...
        {isAdmin && <>
          ...
          <NavBtn id="admin-overview" icon="ti-dashboard"  label="Overview" />
      ...
      <div className="app-shell">
        ...
        <div className="app-content">
          {treeView ? (
            ...
            {view==="master-villages" && <MasterDataView   type="villages"/>}
            {view==="master-surnames" && <MasterDataView   type="surnames"/>}
